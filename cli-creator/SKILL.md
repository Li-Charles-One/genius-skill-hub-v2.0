---
name: cli-creator
description: Build a composable CLI for AI agents from API docs, an OpenAPI spec, existing curl examples, an SDK, a web app, an admin tool, or a local script. Use when the user wants to create a durable command-line tool that runs from any repo, exposes composable read/write commands, returns stable JSON, manages auth, and pairs with a companion skill.
---

# CLI Creator

Create a real CLI that future AI agent sessions can invoke by command name from any working directory.
...
- Default to **Rust** for a durable CLI AI agents should run from any repo: one fast binary, strong argument parsing, good JSON handling, easy copy/install into `~/.local/bin`.
...
After the CLI works, create or update a small skill for it. Use the skill-creator tool when available. Place the companion skill where the target agent looks for skills (check the agent's install path convention — see the Platform Adapter Design section in genius-skill-creator for a reference table).
