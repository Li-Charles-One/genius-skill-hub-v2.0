...
- **Layout**: Document max-width container (max-w-[1400px] or max-w-7xl), breakpoints (sm 640, md 768, lg 1024, xl 1280, 2xl 1536). Name a page rhythm / macrostructure. Viewport stability: min-h-[100dvh] never h-screen. Grid over flex-math: CSS Grid for 2D, Flexbox for 1D, never complex flexbox percentage math. Hero constraints: headline <=2 lines, subtext <=20 words/<=4 lines, max 4 text elements (eyebrow OR brand strip + headline + subtext + CTAs). Forbidden layout patterns: unnamed hero/3-cards/CTA/footer default, hanging tag-left heading-right, centered hero when VARIANCE > 4, 3 equal cards, zigzag beyond 2 consecutive, eyebrow on >1/3 of sections, split-header as default.
...
Read `reference/anti-patterns.md`. Inject all four Universal warnings first. Then match the brand to its category (a brand may belong to multiple categories -- e.g., Linear = Developer Tools + Dark Mode + Minimalist). Merge the category warnings and pick the 3-5 most relevant. Inject as:
...
**Third-order (structure):** If the page outline is still hero / three features / CTA / footer with no named macrostructure, name a different page rhythm and rewrite the Layout section. Color uniqueness is not enough.

If any check fails, redo color, direction, or page rhythm. This is not optional.
